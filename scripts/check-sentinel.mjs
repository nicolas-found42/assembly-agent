...
//
//   * the campaign sentinel (or its stem), or
...
/** The synthetic secret every retained-output check is about (R07). Assembled from
 *  fragments on purpose: if the value existed as a literal in this repository, any
 *  retained copy of that source (a Playwright trace, an injected review diff, an
 *  error context) would be reported as a leak by this very scan. `test/ci-guards.test.mjs`
 *  fails if the literal or the stem appears in any tracked file. */
const SENTINEL = ['SYNTHETIC', 'SECRET', 'SENTINEL'].join('-') + '-' + ['9f3c', '1a'].join('');
/** The stem alone is still the secret leaving the test boundary. */
const SENTINEL_STEM = SENTINEL.slice(0, SENTINEL.lastIndexOf('-'));
