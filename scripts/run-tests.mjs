...
function runEntry(entry, cls, timeout) {
 ...
 // The suite command spawns a tree (npx → playwright → web servers), and
 // Node's built-in timeout only SIGTERMs the direct child — a killed run left
 // its fixture servers bound to 4319/4320 and the next run failed with
 // "already used". Give the child its own process group so a timeout can sweep
 ...
 if (timedOut && res.pid) {
  // The leader is already SIGTERMed by Node; the group members are not. If
  // everything is already gone this raises ESRCH, which is the success case.
...
function main() {
 ...
 for (const entry of entries) {
  // An entry may declare its own bound; the default exists to catch a hung
  // required class, not to fit the slowest optional suite. The effective
  // value is printed so a timeout failure states the bound it hit.
  const timeout = Number.isInteger(entry.timeout) && entry.timeout > 0 ? entry.timeout : args.timeout;
  console.log(`\n▶ ${entry.path} [${entry.run}]${timeout !== args.timeout ? ` timeout=${timeout}ms` : ''}`);
  const result = runEntry(entry, args.class, timeout);
  results.push(result);
  if (result.ok) {
