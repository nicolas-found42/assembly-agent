...
- **JINA WEB/NEWS with an empty reader body (2026-09-15)** — `r.jina.ai` answers 200 with an empty document when it extracts nothing (blocked target, empty result page, limiter page). `jinaHelper` used to build a block anyway: it carried no evidence, but the answer footer counted it as a Source, so a search that produced nothing usable rendered as `Sources (1)` and the documented "Web search was not available for this answer." state was unreachable. It now returns `''` like any other Source that found nothing ("a Source that does not fire simply contributes no block"), which makes the empty state reachable and is pinned by a regression in `test/sources.test.mjs`. No attribution footer is emitted because no Jina content is shown.
