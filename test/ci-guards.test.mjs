...
test('no tracked file carries the sentinel, so a retained copy of the source cannot trip the scan', () => {
  // Learned the hard way on the hosted runner: a third-party review reporter embeds the
  // PR's `git diff` inside the retained Playwright JSON report, so a document or test
  // that quoted the sentinel made `check-sentinel` fail on an otherwise green tree. The
  // value is therefore assembled from fragments wherever it is handled — this test keeps
  // it that way, which is also what makes the scan's own retained-output rule honest.
  const sentinel = ['SYNTHETIC', 'SECRET', 'SENTINEL'].join('-') + '-' + ['9f3c', '1a'].join('');
  const stem = sentinel.slice(0, sentinel.lastIndexOf('-'));
  const needles = [Buffer.from(sentinel), Buffer.from(stem)];
  const listed = spawnSync('git', ['ls-files', '-z'], { cwd: ROOT, encoding: 'utf8' });
  assert.equal(listed.status, 0, 'git ls-files must work here; the gate already assumes a git checkout');
  const hits = [];
  for (const rel of listed.stdout.split('\0')) {
    if (!rel) continue;
    const bytes = readFileSync(path.join(ROOT, rel));
    if (needles.some((needle) => bytes.includes(needle))) hits.push(rel);
  }
  assert.deepEqual(hits, [], `the sentinel must never be a literal in a tracked file: ${hits.join(', ')}`);
});
