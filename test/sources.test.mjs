...
// Real reader render of lite.duckduckgo.com/lite/?q=…: numbered results whose
// href is DDG's uddg redirect wrapper, then a description line and a bare
// display-URL line. js/search.js parses one fmt block per result.
function jinaSerp(q, url = 'https://example.com/result') {
  const target = `https://lite.duckduckgo.com/lite/?q=${encodeURIComponent(q)}`;
  return `Title: ${q} at DuckDuckGo\n\nURL Source: ${target}\n\nMarkdown Content:\n`
    + `1.[${q} — example result](https://duckduckgo.com/l/?uddg=${encodeURIComponent(url)}&rut=9f2a1c3d)\n`
    + `${q} reference page\n${new URL(url).host}\n`;
}
// Real reader render of the Google News RSS search for q.
function jinaNews(q) {
  return `Title: "${q}" - Google News\n\nURL Source: https://news.google.com/rss/search?q=${encodeURIComponent(q)}\n\nMarkdown Content:\n`
    + `[${q}: something big happened - Example News](https://news.google.com/rss/articles/CBMiabcdefgh)\n`;
}
...
// ── smartSlice relevance filter (runs regardless of budget) ──
{
  const scored = fmt('JINA WEB', 'alpha result', 'https://alpha.example', 'alpha alpha');
  const noise = fmt('JINA WEB', 'nothing relevant', 'https://noise.example', 'unrelated words here');
  // under budget: the zero-overlap block is still dropped once one block scores
  const filtered = smartSlice(noise + scored, 'alpha', 20000);
  assert.ok(filtered.includes('https://alpha.example'), 'scored block kept');
  assert.ok(!filtered.includes('https://noise.example'), 'zero-overlap block dropped under budget');
  // over budget: same rule
  const big = noise + scored + Array.from({ length: 60 }, (_, i) => fmt('WIKIPEDIA', `F${i}`, `https://f${i}.example`, `alpha filler ${i}`)).join('');
  const sliced = smartSlice(big, 'alpha', 900);
  assert.ok(!sliced.includes('https://noise.example'), 'zero-overlap block dropped over budget');
  assert.ok(sliced.length <= 900, 'budget respected');
  // no block scores -> the top block is kept rather than an empty slice
  const noneScored = smartSlice(noise + fmt('WIKIPEDIA', 'other', 'https://other.example', 'still unrelated'), 'qwertyuiop', 20000);
  assert.ok(noneScored.includes('https://noise.example'), 'no scored block keeps the blocks');
  assert.ok(noneScored.length > 0, 'non-empty input never yields an empty slice');
  // single zero-overlap block with no scored sibling
  assert.ok(smartSlice(noise, 'qwertyuiop', 20000).includes('https://noise.example'), 'lone zero-overlap block survives');
}
...
// espn network-error tolerated
{
  ...
  const transport = makeFake({
    ...
    'r.jina.ai/https://lite.duckduckgo.com': jinaSerp('nfl game')
  });
...
// jinaweb under limiter happy (always eligible behind limiter)
{
  ...
  const counts = {};
  const transport = makeFake({
    'r.jina.ai/https://lite.duckduckgo.com': jinaSerp('any random query'),
    'endoflife.date/api/all.json': []
  }, counts);
  ...
  assert.ok(r.markdown.includes('https://example.com/result'), 'the real destination URL is the block URL');
  assert.ok(!r.markdown.includes('uddg='), 'the uddg redirect wrapper is never reported as the source URL');
}
{
...
{
  const newsFake = (q) => makeFake({
    'r.jina.ai/https://news.google.com': jinaNews(q),
    'r.jina.ai/https://lite.duckduckgo.com': jinaSerp(q)
  });
  reset();
  const r1 = await webSearch('what happened right now', { transport: newsFake('what happened right now') });
  assert.ok(r1.markdown.includes('### [JINA NEWS]'), '"right now" fires JINA NEWS');
  reset();
  const r2 = await webSearch('tech headlines this week', { transport: newsFake('tech headlines this week') });
  assert.ok(r2.markdown.includes('### [JINA NEWS]'), '"this week" fires JINA NEWS');
  reset();
  const r3 = await webSearch('scores today', { transport: newsFake('scores today') });
...
// ── DDG IA happy / flat RelatedTopics ──
{
  reset();
  const fake = makeFake({
    'api.duckduckgo.com': { AbstractText: '', Answer: '', Heading: 'Duck', AbstractURL: 'https://duckduckgo.com/?q=python', AbstractSource: 'DuckDB', RelatedTopics: [{ Text: 'python programming A', Result: '<a href="https://example.com/a">python programming A</a>', FirstURL: 'https://example.com/a' }, { Topics: [{ Text: 'python programming B nested', Result: '<a>python programming B</a>', FirstURL: 'https://example.com/b' }] }] }
  });
...
// ── MWMBl ?s= param correctness ──
{
  ...
  assert.equal(counts3['api.mwmbl.org/search/?s='] || 0, 0, 'mwmbl q<3 no fetch');
   // live payload shape: an object (not a string) in the extract fields must
   // never render as "[object Object]" — the block carries title/url only.
   reset();
   const objFake = makeFake({ 'api.mwmbl.org/search/?s=': {results:[{title:'T3', url:'https://example.com/m3', extract:{meta:'payload'}}]} });
   const rObj = await webSearch('python programming', {transport: objFake});
   assert.ok(rObj.markdown.includes('### [MWMBl]'), 'object-extract mwmbl block kept');
   assert.ok(!rObj.markdown.includes('[object Object]'), 'mwmbl never renders "[object Object]"');

   // an Openverse result with no title falls back to a non-empty title
   reset();
   const openFake = makeFake({ 'api.openverse.org': {results:[{foreign_landing_url:'https://example.com/cat-park', creator:'Bob cat', license:'cc0'}]} });
   const rOpen = await webSearch('cat', {transport: openFake});
   assert.ok(rOpen.markdown.includes('### [OPENVERSE] Openverse image'), 'openverse title falls back to "Openverse image"');
}
...
// ── 28-job Fan-out integration ──
{
  ...
  // Fixtures carry the query's own terms: smartSlice filters zero-overlap blocks.
  const fake = makeFake({
    'api.duckduckgo.com': { AbstractText: '', Answer: '', RelatedTopics: [{ Text: 'python programming guide', Result: '<a>python programming guide</a>', FirstURL: 'https://example.com/a' }] },
    'w/api.php?action=opensearch': ['q', ['Ada', 'Ada2'], ['', ''], ['https://en.wikipedia.org/wiki/Ada', 'https://en.wikipedia.org/wiki/Ada2']],
    'api/rest_v1/page/summary/Ada': { extract: 'python programming cat image summary', type: 'standard' },
    'api/rest_v1/page/summary/Ada2': { extract: 'python programming cat image summary2', type: 'standard' },
    ...
    'api.mwmbl.org/search/?s=': [{ title: 'Mwmbl T', url: 'https://example.com/m', extract: 'python programming cat image snip' }]
  });
...
// ── intent routing: plan opt + default classifyIntent ──
{
  reset();
  const counts = {};
  await webSearch('alpha beta', {
    transport: makeFake({
      'api.openalex.org': { results: [] },
      'api.crossref.org': { message: { items: [] } },
      'doaj.org': { results: [] }
    }, counts), plan: { kind: 'academic' }
  });
  assert.ok((counts['api.openalex.org'] || 0) > 0, 'academic plan reaches OPENALEX');
  assert.ok((counts['api.crossref.org'] || 0) > 0, 'academic plan reaches CROSSREF');
  assert.ok((counts['doaj.org'] || 0) > 0, 'academic plan reaches DOAJ');
  ...
  await webSearch('alpha beta', {
    transport: makeFake({
      ...
      'lobste.rs': []
    }, counts2), plan: { kind: 'general' }
  });
  assert.equal(counts2['api.openalex.org'] || 0, 0, 'general plan skips OPENALEX');
  assert.equal(counts2['api.crossref.org'] || 0, 0, 'general plan skips CROSSREF');
  assert.equal(counts2['doaj.org'] || 0, 0, 'general plan skips DOAJ');
  assert.equal(counts2['hn.algolia.com'] || 0, 0, 'general plan skips HACKER NEWS');
  assert.equal(counts2['api.github.com'] || 0, 0, 'general plan skips GITHUB');
  assert.equal(counts2['api.stackexchange.com'] || 0, 0, 'general plan skips STACK OVERFLOW');
  assert.equal(counts2['lobste.rs'] || 0, 0, 'general plan skips LOBSTE.RS');

  reset();
  const counts3 = {};
  await webSearch('alpha beta', {
    transport: makeFake({
      'hn.algolia.com': { hits: [] }, 'api.github.com': { items: [] }, 'api.stackexchange.com': { items: [] }, 'lobste.rs': []
    }, counts3), plan: { kind: 'code' }
  });
  assert.ok((counts3['hn.algolia.com'] || 0) > 0, 'code plan reaches HACKER NEWS');
  assert.ok((counts3['api.github.com'] || 0) > 0, 'code plan reaches GITHUB');
  assert.ok((counts3['api.stackexchange.com'] || 0) > 0, 'code plan reaches STACK OVERFLOW');
  assert.ok((counts3['lobste.rs'] || 0) > 0, 'code plan reaches LOBSTE.RS');
}
console.log('routing plan PASS');

{
  // default routing uses classifyIntent(query): academic query yes, visual query no
  reset();
  const counts = {};
  await webSearch('quantum entanglement arxiv paper', { transport: makeFake({ 'api.openalex.org': { results: [] } }, counts) });
  assert.ok((counts['api.openalex.org'] || 0) > 0, 'academic query reaches OPENALEX by default routing');
  reset();
  const counts2 = {};
  await webSearch('cat picture', { transport: makeFake({ 'api.openalex.org': { results: [] } }, counts2) });
  assert.equal(counts2['api.openalex.org'] || 0, 0, 'visual query does not reach OPENALEX');
}
console.log('routing default PASS');

{
  // news intent adds JINA NEWS + CURRENT EVENTS; without it the token gate decides
  reset();
  const counts = {};
  await webSearch('weekly roundup', {
    transport: makeFake({
      'r.jina.ai/https://news.google.com': jinaNews('weekly roundup'),
      'w/api.php?action=parse': { parse: { text: { '*': '<ul><li>Something happened</li></ul>' } } }
    }, counts), plan: { kind: 'news' }
  });
  assert.ok((counts['r.jina.ai/https://news.google.com'] || 0) > 0, 'news plan reaches JINA NEWS without a news token');
  assert.ok((counts['w/api.php?action=parse'] || 0) > 0, 'news plan reaches CURRENT EVENTS without a news token');
  ...
  assert.equal(counts2['r.jina.ai/https://news.google.com'] || 0, 0, 'general plan + no news token skips JINA NEWS');
  assert.equal(counts2['w/api.php?action=parse'] || 0, 0, 'general plan + no news token skips CURRENT EVENTS');
}
console.log('routing news PASS');

{
  // ESPN requires scoreboard tokens as well as a league token or team nickname:
  // the NBA career-points question is not a scoreboard query.
  reset();
  let espnCalled = false;
  const transport = async (url) => {
    if (url.includes('site.api.espn.com')) espnCalled = true;
  ...
  const r = await webSearch('who has the most points in nba history how many points od they have', { transport });
  assert.equal(espnCalled, false, 'career-points question is not a scoreboard query');
  assert.ok(!r.markdown.includes('### [ESPN]'), 'no ESPN block for a career-stat question');
}
console.log('espn score gate PASS');
