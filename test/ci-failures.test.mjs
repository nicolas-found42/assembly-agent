...
test('check-sentinel: a planted sentinel or foreign key fails the scan and is named', async () => {
  ...
  try {
    ...
    // Both values are assembled rather than written as literals: this file's own
    // source must never carry them (a retained copy of it — a trace, an injected
    // review diff — would otherwise be reported as a leak by the scan it drives).
    const sentinel = ['SYNTHETIC', 'SECRET', 'SENTINEL'].join('-') + '-' + ['9f3c', '1a'].join('');
    const foreignKey = ['sk', 'or', 'v1', 'ABCDEFGHIJKLMNOP1234'].join('-');
    writeFileSync(leak, ['# retained run log', `api key ${sentinel} must never ship`, ''].join('\n'));
    writeFileSync(foreign, `authorization: ${foreignKey}\n`);
